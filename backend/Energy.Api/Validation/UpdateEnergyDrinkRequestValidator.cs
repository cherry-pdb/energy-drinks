...
public sealed class UpdateEnergyDrinkRequestValidator : AbstractValidator<UpdateEnergyDrinkRequest>
{
    public UpdateEnergyDrinkRequestValidator()
    {
        ...
        RuleFor(x => x.CaffeineMg).InclusiveBetween(0m, 5000m).When(x => x.CaffeineMg.HasValue);
        RuleFor(x => x.SugarGrams).InclusiveBetween(0m, 5000m).When(x => x.SugarGrams.HasValue);
        RuleFor(x => x.Calories).InclusiveBetween(0m, 10000m).When(x => x.Calories.HasValue);
